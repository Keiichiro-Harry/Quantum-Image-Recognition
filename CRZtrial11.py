...
def function_n2(n,m,ki,cycle):
    ...
    for i in range(2**m):
        ...
        # print(f"今ここ{i}")
        MCRZ(FLAG2[i],n+m,state,kw_128[i+cycle*64])#qubit数変えるときここも変える
        # print(f"終わった{i}")
        for j in range(m):
            if(FLAG3[i][j]==0):
                X(n-m+j).update_quantum_state(state)
...
kw_128=np.array([0.0 for j in range (128)])
# kw_128=[ 0.0937922,   0.02135668,  0.07413795, -0.06516433, -0.04831931,  0.02232466,
...
# kw_4[1]=-1
kw_4=[0.05288195, 0.49724038, 0.09254522, 0.35010126]
# kw_4[3]=-1
s_128=np.zeros((100,128))
s_4=np.zeros((100,4))
for i in range(100):
    for j in range(128):
        s_128[i][j]=1-2*scipy.stats.bernoulli.rvs(0.5)
    for j in range(4):
        s_4[i][j]=1-2*scipy.stats.bernoulli.rvs(0.5)

def cost_calculator(i):
    global kw_4, kw_128, s_128, s_4
...
def main_calculation(i):
    ...
    c2=1/50
    eta=1/(1000+10*i)
    kw_128=kw_128+s_128[i]*c1
    kw_4=kw_4+s_4[i]*c2
    result2[i]=main_cycle()
    kw_128=kw_128-s_128[i]*c1-(result2[i]-result1[i])*s_128[i]*eta/c1
...
def one_cycle(input):
    num_ones=np.array([0.0 for i in range(6)])
    original_num_ones=function_n2(8,6,input,0)
    # print(original_num_ones)
    num_ones=(original_num_ones-np.average(original_num_ones))*2*pi/sum(original_num_ones)
    # print(num_ones)
    ki2=np.array([np.e**(1j*0)/32 for i in range(1024)])
    ki2[5]=ki2[5]*(np.e**(1j*num_ones[0]))
    ki2[6]=ki2[6]*(np.e**(1j*num_ones[1]))
    ki2[7]=ki2[7]*(np.e**(1j*num_ones[2]))
    ki2[8]=ki2[8]*(np.e**(1j*num_ones[3]))
    ki2[9]=ki2[9]*(np.e**(1j*num_ones[4]))
    ki2[10]=ki2[10]*(np.e**(1j*num_ones[5])) 
    ki2[18]=ki2[18]*(np.e**(1j*num_ones[0]))
    ki2[21]=ki2[21]*(np.e**(1j*num_ones[1]))
    ki2[22]=ki2[22]*(np.e**(1j*num_ones[2]))
    ki2[23]=ki2[23]*(np.e**(1j*num_ones[3]))
    ki2[24]=ki2[24]*(np.e**(1j*num_ones[4]))
    ki2[25]=ki2[25]*(np.e**(1j*num_ones[5]))
    ki2[45]=ki2[45]*(np.e**(1j*num_ones[0]))
    ki2[34]=ki2[34]*(np.e**(1j*num_ones[1]))
    ki2[37]=ki2[37]*(np.e**(1j*num_ones[2]))
    ki2[38]=ki2[38]*(np.e**(1j*num_ones[3]))
    ki2[39]=ki2[39]*(np.e**(1j*num_ones[4]))
    ki2[40]=ki2[40]*(np.e**(1j*num_ones[5]))
    ki2[58]=ki2[58]*(np.e**(1j*num_ones[0]))
    ki2[61]=ki2[61]*(np.e**(1j*num_ones[1]))
    ki2[50]=ki2[50]*(np.e**(1j*num_ones[2]))
    ki2[53]=ki2[53]*(np.e**(1j*num_ones[3]))
    ki2[54]=ki2[54]*(np.e**(1j*num_ones[4]))
    ki2[55]=ki2[55]*(np.e**(1j*num_ones[5]))
    for i in range(64):
        for j in range(16):
