...
def test_multi_cycle(n):
    p = Pool(32)
    result0 = p.map(test_cycle_zero, range(n))
    result1 = p.map(test_cycle_one, range(n))
    # for i in range(n):
    #     ki_processor(i)
    return n,result0,result1

def test_main_cycle():
    i,data0, data1 = test_multi_cycle(100)
    for j in data0:
        print(j)
    for j in data1:
        print(j)
...
def main_calculation(i):
    ...
    if(0<i and i<99):
        ...
        kw_272=kw_272-LOSS*s_272[i]/(i/10+10)/100
...
def test_cycle_zero(i):
    return np.dot(one_cycle(ki0[i]),kw_4)
def test_cycle_one(i):
    return np.dot(one_cycle(ki1[i]),kw_4)
