...
def function_n2(n,m,ki,cycle):
    ...
    for i in range(2**m):
        ...
        # print(f"今ここ{i}")
        if(cycle==0):
            MCRZ(FLAG2[i],n+m,state,kw_32[i])
        else:
            MCRZ(FLAG2[i],n+m,state,kw_32[i+16])#qubit数変えるときここも変える
        # print(f"終わった{i}")
        for j in range(m):
            if(FLAG3[i][j]==0):
                X(n-m+j).update_quantum_state(state)
...
def main_calculation(i):
    ...
    if(0<i and i<99):
        ...
        kw_32=kw_32-LOSS*s_32[i]/(i/10+10)/100
...
def one_cycle(input):
    ...
    original_num_ones=function_n2(6,4,input,0)
...
#このコードはパディングしなかったらどうなるのかを検証するためのものです。
